...
```
Settings (persisted as variables on the player's PRIMARY TITLE, so they
survive succession — the heir inherits the title and its variables):
   ...
   └── eotr_apply_all_vassals_for_player               (scripted effect)
        └── every_vassal_or_below (landed AI, ≥ county tier)
             └── eotr_evaluate_and_assign_directive   (scripted effect)
                  ├── reads scope:eotr_player.primary_title's setting variables
...
- [common/on_action/eotr_on_actions.txt](common/on_action/eotr_on_actions.txt) — auto-apply hooks: `yearly_global_pulse`, `on_title_gain`/`on_title_lost`, `on_character_faith_change`/`on_character_culture_change`, `on_county_faith_change`/`on_county_culture_change`. All gated on the realm's top liege being a player with `eotr_auto_apply` set. PLUS one non-auto-apply hook, `on_primary_title_change`, which is purely a settings-storage migration (see below) — it does not apply directives and is NOT one of the bounded auto-apply triggers.
...
- **Settings persist as variables on the player's primary title** (`primary_title = { set_variable = eotr_... }`), NOT as character flags. This is what makes them survive succession: character flags die with the character, but title variables transfer to the heir along with the title. Each setting decision clears all sibling variables in its category (`primary_title = { remove_variable = ... }`), then sets the chosen one. To add a new option to an existing setting, add the `remove_variable` to the decision's `effect` clearing block AND add a new `else_if = { limit = { scope:..._opt = yes } primary_title = { set_variable = ... } }` branch. NEVER use `scope:X ?= { ... }` on the option scope — `?=` shifts scope into the option-list boolean tag (not a character), so the write silently fails to persist; use `if + limit = { scope:X = yes }` with `primary_title = { set_variable = ... }` inside (the surrounding scope there is the player character, on whom `primary_title` resolves). All setting reads go through `primary_title = { has_variable = ... }`. (The vassal-side `vassal_directive_*` OUTPUTS remain vanilla character flags — only the player's SETTINGS moved to title variables.) Ordinary succession is covered for free (the heir inherits the primary title and its variables); a change of *which* title is primary (e.g. forming an empire) is covered by the `on_primary_title_change` hook, which copies the variables from `scope:previous_title` onto the new `primary_title` via `eotr_migrate_settings_to_primary_title` and clears the source. When adding a new setting variable, also add it to that migration effect's copy/clear lists AND to the `eotr_title_has_settings` scripted trigger that gates the migration.
...
- **Scope contract for the evaluator:** `eotr_evaluate_and_assign_directive` runs on a vassal scope. Every caller MUST `save_scope_as = eotr_player` on the player (the one whose settings drive the assignment) before invoking it — root varies wildly across call sites (player from the manual decision, county/vassal/unset from on_actions) so the saved scope is the only stable handle. All reads of the player's settings must use `scope:eotr_player = { primary_title = { has_variable = ... } }` (settings live as title variables, not character flags). All checks of the vassal's own faith/culture/lands use the implicit (vassal) scope.
- **Auto-apply hooks gate on the realm's top liege.** Every on_action effect in [common/on_action/eotr_on_actions.txt](common/on_action/eotr_on_actions.txt) starts with a filter of the form `top_liege = { is_ai = no, primary_title = { has_variable = eotr_auto_apply } }` (or `holder.top_liege = { ... }` for county hooks). This rejects nearly every fire cheaply (most realms don't belong to an auto-apply player) and ensures other players' realms — and AI realms — are never touched. Never assume the realm-top is a player without checking.
